...
The app loads your web page inside a native container and talks to it over a small **JSON message bridge**. This document is everything you need to build the page so it behaves like a native screen: navigation into the app, native share, native reminders, analytics, and hardware-back handling.
...
> ⚠️ The app end is on branch `feature/kundali-webview-viewer` (PR #5241). A few values are still being finalized (see [Open items](#12-open-items-coordinate-with-the-app-team)).
...
- It runs with **no auth token** (the page fetches the kundali by the `orderId` the app passes in `init`).
...
AppBridge.onInit((init) => {
  // init = { locale, platform, data: { orderId, contentType }, auth? }
  renderKundali(init.data.orderId, init.locale);
});
...
| `action` | `{ uri, data? }` (+ `id`) | yes (if `id`) | Run a native action: share / reminder (see §8). |
...
```jsonc
{
  ...
  "data": {                  // consumer-specific — for kundali:
    "orderId": "TEST123",    //   the order whose kundali to render (fetch by this)
    "contentType": "kundali"
  },
  "auth": { "token": "..." } // ONLY present if the consumer needs auth — ABSENT for kundali
}
```
> Render using `data.orderId`. Don't assume `auth` exists for kundali.
...
### Adding new actions
Only `share` and `reminder` exist today. New actions require an app-side change + allow-list entry — coordinate with the app team.
...
- You can also proactively `AppBridge.close()` (e.g. an on-page ✕). Because the kundali screen has **no native app bar**, provide a visible back/close control in your page — essential on iOS.
